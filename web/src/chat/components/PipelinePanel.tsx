  ...
  return (
    <aside id="memory-pipeline-panel" className={classNames('memory-pipeline-panel', Boolean(snapshot) && 'open')} aria-hidden={!snapshot}>
      ...
      <div id="memory-pipeline-panel-body" className="memory-pipeline-panel-body">
        <section className="memory-pipeline-section"><div className="memory-pipeline-section-title">prompt_block</div><pre className="memory-pipeline-prompt">{stringField(snapshot, 'prompt_block') || stringField(snapshot, 'promptBlock') || '(empty)'}</pre></section>
        ...
        <section className="memory-pipeline-section"><div className="memory-pipeline-section-title">stages</div>{stages.map(stage => <div className="memory-pipeline-stage" key={stage.key}><div className="memory-pipeline-stage-name">{stage.key}</div>{stage.items.length ? stage.items.map((item, index) => <div className="memory-pipeline-item" key={index}><span>{stringField(item, 'content_summary') || stringField(item, 'contentSummary') || '(empty)'}</span><span className="memory-pipeline-score">{String(field(item, 'score', ''))}</span></div>) : <div className="memory-pipeline-empty">无记录</div>}</div>)}</section>
      </div>
    </aside>
  );
}
