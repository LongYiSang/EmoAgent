...
export function SlotEditor({ id, label, draft, providers, providerPresets, models, onDraft }: { id: string; label: string; draft: AgentConfig; providers: Provider[]; providerPresets: ProviderPreset[]; models: string[]; onDraft: (draft: AgentConfig) => void }) {
  ...
  return (
    <div className="slot" data-slot={id}>
      <div className="slot-head"><strong>{label}</strong><button className="btn ghost mini" type="button" onClick={applyRecommendedParams}>Apply recommended</button></div>
      <div className="grid compact">
        <div className="field"><label htmlFor={`${id}-provider`}>Provider</label><select id={`${id}-provider`} value={String(binding.provider_id || '')} onChange={event => setBinding(['provider_id'], event.target.value)}><option value="">Select provider</option>{providers.map(provider => <option key={provider.id} value={provider.id}>{provider.name || provider.id}</option>)}</select></div>
        <Field id={`${id}-model`} label="Model" value={String(binding.model || '')} onChange={value => setBinding(['model'], value)} list="model-options" />
        <div className={classNames('field', paramMeta('max_tokens').hidden && 'hidden-param')}><label htmlFor={`${id}-max`}>Max Tokens</label><input id={`${id}-max`} type="number" value={String(params.max_tokens ?? '')} onChange={event => setBinding(['params', 'max_tokens'], toInt(event.target.value))} />{renderNote('max_tokens')}</div>
        ...
        <div className={classNames('field', paramMeta('stream').hidden && 'hidden-param')}><label htmlFor={`${id}-stream`}>Stream</label><select id={`${id}-stream`} value={params.stream === undefined ? '' : String(params.stream)} onChange={event => setBinding(['params', 'stream'], event.target.value === '' ? undefined : event.target.value === 'true')}><option value="">inherit</option><option value="true">true</option><option value="false">false</option></select>{renderNote('stream')}</div>
        ...
        <div className={classNames('field', paramMeta('reasoning_effort').hidden && 'hidden-param')}><label htmlFor={`${id}-reasoning`}>Reasoning Effort</label><input id={`${id}-reasoning`} value={String(params.reasoning_effort ?? '')} onChange={event => setBinding(['params', 'reasoning_effort'], event.target.value)} />{renderNote('reasoning_effort')}</div>
        <div className={classNames('field', paramMeta('thinking_mode').hidden && 'hidden-param')}><label htmlFor={`${id}-thinking-mode`}>Thinking Mode</label><input id={`${id}-thinking-mode`} value={String(field(field(params, 'thinking', {}), 'mode', ''))} onChange={event => setBinding(['params', 'thinking', 'mode'], event.target.value)} />{renderNote('thinking_mode')}</div>
        <div className={classNames('field', paramMeta('thinking_budget').hidden && 'hidden-param')}><label htmlFor={`${id}-thinking-budget`}>Thinking Budget</label><input id={`${id}-thinking-budget`} type="number" value={String(field(field(params, 'thinking', {}), 'budget_tokens', ''))} onChange={event => setBinding(['params', 'thinking', 'budget_tokens'], toInt(event.target.value))} />{renderNote('thinking_budget')}</div>
        <div className={classNames('field', paramMeta('thinking_effort').hidden && 'hidden-param')}><label htmlFor={`${id}-thinking-effort`}>Thinking Effort</label><input id={`${id}-thinking-effort`} value={String(field(field(params, 'thinking', {}), 'effort', ''))} onChange={event => setBinding(['params', 'thinking', 'effort'], event.target.value)} />{renderNote('thinking_effort')}</div>
      </div>
      <div className={classNames('field', paramMeta('extra').hidden && 'hidden-param')}><label htmlFor={`${id}-extra`}>Extra JSON</label><textarea id={`${id}-extra`} value={extraDraft} onChange={event => { setExtraDraft(event.target.value); setExtraError(''); }} onBlur={commitExtraDraft} />{extraError && <div className="field-error">{extraError}</div>}{renderNote('extra')}</div>
