...
export default memo(function PipelinesTab({ providers, memoryDraft, updateMemoryPath, savePipelines }: PipelinesTabProps) {
  ...
  return (
    <div className="section">
      <div className="hero"><div><h2>Pipelines</h2><div className="meta">Provider/model bindings are selected by provider_id + model</div></div><button className="btn primary" id="save-pipelines" type="button" onClick={savePipelines}>Save</button></div>
        ...
        {memoryPipelineBindings.map(([key, label]) => {
          ...
          return (
            <div className="pipeline-row" key={key}>
              ...
              {llmPipelineKeys.has(key) && <div className="field"><label htmlFor={`mem-${key}-thinking`}>{label} Thinking</label><select id={`mem-${key}-thinking`} value={String(field(field(binding, 'thinking', {}), 'type', ''))} onChange={event => setBinding(key, ['thinking', 'type'], event.target.value)}>{pipelineThinkingOptions(String(field(field(binding, 'thinking', {}), 'type', ''))).map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>}
            </div>
          );
        })}
      </div>
