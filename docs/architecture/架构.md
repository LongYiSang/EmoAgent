...
`resume_work` 继续采用增量回传语义：只追加新的决策结果，只传递必要的约束变化，不重发完整任务和完整人格。当前实现把恢复调用拆成两类：普通决策和 `permission_escalation_required` 仍由 Emotion 发起；`tool_approval` 则在用户点击审批后由系统执行层直接带 `approval_request_id` 续跑，再把恢复结果交给 Emotion 组织对外回复。
