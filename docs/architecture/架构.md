...
这样做的目的是让 Work 保持执行效率，同时保留 Emotion 对任务意图和表达风格的控制权。
...
```
Emotion ──TaskBrief──────→ Work     任务契约：目标、背景、约束、验收标准

Work ────DecisionRequest──→ Emotion  决策升级：问题、阻塞原因、可选方案

Emotion ──DecisionResponse─→ Work    增量决策：append-only, delta-only
                                     不重发完整任务，不重发完整 persona

Work ────TaskReport───────→ Emotion  结果报告：状态、摘要、关键发现、遗留问题
```

**DecisionResponse 的核心设计约束：**

- `append-only` —— 仅追加新的决策结果
- `delta-only` —— 仅传递增量约束

Work 收到 DecisionResponse 后继续在原上下文中执行，而不是重启新任务。
...
TaskBrief 协议、Work 自循环执行（基于已有的工具系统）、执行日志写入、TaskReport 返回、DecisionRequest / DecisionResponse、Emotion 决策处理、必要时向用户澄清。
...
| DecisionRequest 过频 | Emotion 成为瓶颈，响应延迟 | 明确自主处理 vs 升级的判断标准 |
...
MVP 的 Work 权限由 TaskBrief 中简单的 allow/deny 控制。后期可引入分级权限体系（read-only → workspace-write → sensitive-write → destructive-action），Work 不得自行升级权限，只能请求 Emotion 审批。
