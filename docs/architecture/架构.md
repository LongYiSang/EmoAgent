...
- 在 `tool_approval` 路径下负责消费系统恢复后的结果并组织对外表达，而不是额外生成一次恢复调用
- 接收 TaskReport，组织最终回复
- 审批并管理长期记忆写入

**Emotion 也可以直接使用轻量工具**，并通过系统上下文获得当前时间；直连工具的前提是：
...
- 已处于 `approved-destructive` 时若命中审批门控，则由 runtime 生成 `tool_approval` 并等待交互层基于 `approval_request_id` 续跑
- 将简洁结果报告返回 Emotion
...
Runtime ─tool_approval────────→ 交互层  审批门控请求：展示审批卡片并等待 approval_action

Emotion ──resume_work──────→ Work     恢复执行：普通决策与提权恢复
系统执行层 ─approval_request_id→ Work  审批门控恢复：用户点击审批后直接续跑
...
`resume_work` 继续采用增量回传语义：只追加新的决策结果，只传递必要的约束变化，不重发完整任务和完整人格。当前实现把恢复调用拆成两类：普通决策和 `permission_escalation_required` 仍由 Emotion 发起；`tool_approval` 则在用户点击审批后由系统执行层直接带 `approval_request_id` 续跑，再把恢复结果交给 Emotion 组织对外回复。若内部恢复已经得到终态 `TaskReport`，后续 Emotion 叙述轮会禁用工具，只负责把结果表达给用户，避免已完成任务被再次委派。

工具权限判定统一由 `Dispatcher.ClassifyCall` 负责：每个工具调用在执行前完成注册表查询、schema 校验、工具自声明的破坏性判定、权限比较和 active approval context 检查。运行时只消费 `CallClassification` 的 action，不再维护额外的 approval / escalation 预判链路。
    ...
    alt 轻量任务
        ...
        loop Work 自循环
            ...
            opt approved-destructive 下命中审批门控
                T-->>W: tool_approval
                S-->>U: approval_required / 审批卡片
                U->>S: approval_action
                S-->>W: resume_work (approval_request_id)
                W-->>S: resumed outcome / next pause
                S-->>E: inject resumed outcome
            end
        end
        W-->>E: TaskReport
    end
...
Tool 定义规范（JSON Schema）、Handler 注册机制、工具调度（dispatch map）、内置基础工具（文件读写、Work 时间查询等）。Emotion 通过当前时间上下文获得时间信息，直连轻量工具在此阶段即可使用。**工具是 Agent 的基础能力层，Emotion 和 Work 都依赖它。**
