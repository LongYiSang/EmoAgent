...
- `human_confirmation` 需要明确的用户确认，通常是高影响或需要人工拍板的场景。
...
- 运行时会对 `finish_task`、`request_decision`、审批拦截路径分别做专门处理。
...
当 Work 产生的普通工具调用需要破坏性权限，而当前权限不足或缺少批准时，运行时不会让 LLM 自己发明一个 `tool_approval`。

运行时会直接：

- 识别被阻塞的工具调用
- 生成 runtime-only 的 `tool_approval` 决策 packet
- 把被阻塞的调用保存到 `PendingToolCall`
- 将 paused work 写入 `PendingRegistry`
- 等待 Emotion 通过 `resume_work` 恢复

恢复时：

- 如果审批被拒绝，运行时会向 Work 注入一个“未执行”的错误工具结果
- 如果审批被批准，运行时会带着批准上下文执行冻结的工具调用

这样可以保证审批链路在运行时闭环完成，而不是依赖模型自行遵守约定。
