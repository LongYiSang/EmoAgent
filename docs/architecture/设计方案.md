...
同样，ProgressUpdate（Work 发给 Emotion 的进度更新，用于长任务状态同步）也可作为后期可选协议引入。
...
**MVP 阶段：** 采用简单的 allow/deny 控制，由 Emotion 在 TaskBrief 中定义。
...
> **远期方向：** 后期可引入分级权限体系（read-only → workspace-write → sensitive-write → destructive-action）。
