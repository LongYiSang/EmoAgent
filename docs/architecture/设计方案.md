...
graph TD
    ...
    E -->|resume_work| W
    U -.->|approval_action| S["系统执行层"]
    S -->|approval_request_id / resume_work| W
...
- 在 `tool_approval` 路径下不额外承担恢复调用本身；系统执行层会在用户点击审批后直接按 `approval_request_id` 续跑 Work，Emotion 负责消费恢复结果并组织对外表达
- 自主做出决策，必要时向用户追问
...
Emotion 也可以直接调用一组轻量工具，并通过系统上下文获得当前时间；这些工具必须满足：
...
- 在 `approved-destructive` 命中审批门控时，由 runtime 暂停并等待交互层基于 `approval_request_id` 直接续跑
...
- `tool_approval`：审批门控的破坏性工具调用，由 runtime 拦截触发，并通过交互层暴露为审批请求

### 8.3 resume_work

`resume_work` 是恢复 Work 的执行契约。语义上它仍属于 Emotion 侧恢复入口，但实际发起方分两类：

- 普通决策 / `permission_escalation_required`：由 Emotion 在理解用户答复后调用
- `tool_approval`：由系统执行层在收到 `approval_action` 后直接携带 `approval_request_id` 调用，随后把恢复结果交回 Emotion
...
Work 在收到 `resume_work` 后，继续在原上下文中执行，而不是重启新任务。对于 `tool_approval`，当前实现不会再额外等待一轮 Emotion LLM 去生成恢复调用，以降低审批后的点击延迟和过期风险。若系统执行层内部恢复后已经得到终态 `TaskReport`，Chat Engine 会让 Emotion 进入无工具叙述轮，只组织对外表达，避免重复委派同一已完成任务。
...
stateDiagram-v2
    ...
    NeedDecision --> Running: 收到 resume_work / approval_request_id 恢复
...
- 工具权限判定统一走 `Dispatcher.ClassifyCall`；运行时根据分类 action 暂停、报错或执行
...
- `tool_approval` 触发后，审批 UI 由交互层直接展示；用户点击后由系统执行层携带 `approval_request_id` 续跑 Work，Emotion 负责恢复后的对外表达
...
- Handler 注册机制
- 工具调度（dispatch map）
- 内置基础工具（文件读写、Work 时间查询等）
- Emotion 通过当前时间上下文获得时间信息；直连轻量工具在此阶段可用
