...
- `Work` 在遇到需要判断、设计取舍、权限升级或用户确认的问题时，向 `Emotion` 发起 `request_decision`
...
| 任务执行模型 | 多轮自循环直到完成 |
| 决策升级 | Work 通过 `request_decision` 向 Emotion 发起请求 |
...
graph TD
    ...
    W --> A["审计产物存储"]
    W -->|request_decision / DecisionPacket / TaskReport| E
    E -->|resume_work| W
...
sequenceDiagram
    ...
    loop Work 执行循环
        ...
        alt 需要普通确认
            ...
            W->>T: 破坏性工具调用
            W->>E: runtime-generated tool_approval
...
Work 在遇到判断阻塞或普通确认时，向 Emotion 发起的升级请求；审批门控的破坏性工具调用则由运行时生成独立的 `tool_approval` packet。
...
- Work 不得自行升级权限
- Work 只能通过 `request_decision` 请求 Emotion 审批权限升级
- `tool_approval` 只用于 runtime 拦截到的审批门控破坏性工具调用
